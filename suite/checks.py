...
def two_values_match(observed: list[str]) -> str | None:
    if len(observed) != 2:
...
def two_values_differ(observed: list[str]) -> str | None:
    if len(observed) != 2:
        return f"expected two reads, got {len(observed)}"
    first, second = observed
    if first == second:
