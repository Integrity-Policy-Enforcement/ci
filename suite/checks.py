...
def two_values_match(values: list[str]) -> str | None:
    if len(values) != 2:
...
def two_values_differ(values: list[str]) -> str | None:
    if len(values) != 2:
        return f"expected two reads, got {len(values)}"
    first, second = values
    if first == second:
