...
def read_case(
    ...
    """Read a file as X509_CERT and check its errno and actual retained bytes."""
    return Case(
        id=id,
        setup=(
        ...
        trigger=partial(x509.read_file, binary=binary),
        checks=(
            partial(checks.errno_is, expected=expected_errno),
            partial(x509.check_contents, expected=expected_content),
        ),
    )
