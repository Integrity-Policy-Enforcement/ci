...
initrd: what boots before the real root, and what it leaves behind.
    ...
    /usr/lib/ipe-tests/                the suite and layout.py; the boot path uses:
        ...
        firmware-boot-verified-false.p7s   deny FIRMWARE when it is false
...
class build:
    ...
    @staticmethod
    def fsverity_signature(algorithm: str, compressed: bool) -> Path:
    ...
    @staticmethod
    def fsverity_digest(algorithm: str, compressed: bool) -> Path:
...
class guest:
    ...
    FSVERITY_PLAIN_SCRIPT_TEST_BINARY = FSVERITY_EXECUTE_DIR / "plain-script"
    FSVERITY_PLAIN_SHEBANG_TEST_SCRIPT = FSVERITY_EXECUTE_DIR / "plain-shebang"
    FSVERITY_PLAIN_PRELOAD_LIBRARY = FSVERITY_EXECUTE_DIR / "plain-preload.so"
    ...
    PLAIN_POLICY_OP_TEST_BINARY = PLAIN_MOUNT_DIR / test_media.POLICY_OP_TEST_BINARY
    PLAIN_X509_TEST_BINARY = PLAIN_MOUNT_DIR / test_media.X509_TEST_BINARY
    PLAIN_EXECUTE_TEST_BINARY = PLAIN_MOUNT_DIR / test_media.EXECUTE_TEST_BINARY
    PLAIN_SCRIPT_TEST_BINARY = PLAIN_MOUNT_DIR / test_media.SCRIPT_TEST_BINARY
    PLAIN_SHEBANG_TEST_SCRIPT = PLAIN_MOUNT_DIR / test_media.SHEBANG_TEST_SCRIPT
    PLAIN_PRELOAD_LIBRARY = PLAIN_MOUNT_DIR / test_media.PRELOAD_LIBRARY
    PLAIN_KEXEC_IMAGE_TEST_BINARY = PLAIN_MOUNT_DIR / test_media.KEXEC_IMAGE_TEST_BINARY
    PLAIN_KEXEC_INITRAMFS_TEST_BINARY = PLAIN_MOUNT_DIR / test_media.KEXEC_INITRAMFS_TEST_BINARY
...
class initrd:
    ...
    EXECUTE_TEST_BINARY = TESTS_DIR / "execute-target"
    EXECUTE_BOOT_VERIFIED_TRUE_ALLOW_POLICY_SIGNATURE = (
        TESTS_DIR / "execute-boot-verified-true.p7s"
    )
    EXECUTE_BOOT_VERIFIED_FALSE_DENY_POLICY_SIGNATURE = (
        TESTS_DIR / "execute-boot-verified-false.p7s"
    )
    KMODULE_TEST_BINARY = TESTS_DIR / _KMODULE_TEST_BINARY_NAME
    FIRMWARE_TEST_BINARY = TESTS_DIR / _FIRMWARE_TEST_BINARY_NAME
    POLICY_OP_TEST_BINARY = TESTS_DIR / test_media.POLICY_OP_TEST_BINARY.name
    POLICY_OP_TEST_MODULE = TESTS_DIR / test_media.POLICY_OP_TEST_MODULE.name
    X509_TEST_BINARY = FSVERITY_CERTIFICATE
    X509_TEST_MODULE = TESTS_DIR / test_media.X509_TEST_MODULE.name
    ...
    POLICY_OP_BOOT_VERIFIED_FALSE_DENY_POLICY_SIGNATURE = (
        TESTS_DIR / "policy-op-boot-verified-false.p7s"
    )
    KEXEC_IMAGE_TEST_BINARY = TESTS_DIR / test_media.KEXEC_IMAGE_TEST_BINARY.name
    KEXEC_INITRAMFS_TEST_BINARY = TESTS_DIR / test_media.KEXEC_INITRAMFS_TEST_BINARY.name
    ...
    FIRMWARE_BOOT_VERIFIED_FALSE_DENY_POLICY_SIGNATURE = (
        TESTS_DIR / "firmware-boot-verified-false.p7s"
    )
    ...
    BOOT_TMPFS_EXECUTE_TEST_BINARY = BOOT_TMPFS_DIR / EXECUTE_TEST_BINARY.name
    BOOT_TMPFS_KMODULE_TEST_BINARY = BOOT_TMPFS_DIR / _KMODULE_TEST_BINARY_NAME
    BOOT_TMPFS_FIRMWARE_TEST_BINARY = BOOT_TMPFS_DIR / _FIRMWARE_TEST_BINARY_NAME
    BOOT_TMPFS_POLICY_OP_TEST_BINARY = BOOT_TMPFS_DIR / test_media.POLICY_OP_TEST_BINARY.name
    BOOT_TMPFS_X509_TEST_BINARY = BOOT_TMPFS_DIR / test_media.X509_TEST_BINARY.name
    BOOT_TMPFS_KEXEC_IMAGE_TEST_BINARY = BOOT_TMPFS_DIR / test_media.KEXEC_IMAGE_TEST_BINARY.name
    BOOT_TMPFS_KEXEC_INITRAMFS_TEST_BINARY = BOOT_TMPFS_DIR / test_media.KEXEC_INITRAMFS_TEST_BINARY.name
